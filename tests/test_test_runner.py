...
def test_test_runner_creation() -> None:
    """Test creating CommandRunner object."""
    runner = CommandRunner("pytest tests/")
...
def test_test_runner_custom_timeout() -> None:
    """Test creating CommandRunner with custom timeout."""
    runner = CommandRunner("make test", timeout=60)
...
def test_test_runner_pass(mock_run: MagicMock) -> None:
    ...
    runner = CommandRunner("echo 'success'")
    result = runner.run()
...
def test_test_runner_error_on_exception(mock_run: MagicMock) -> None:
    ...
    runner = CommandRunner("nonexistent-command")
    result = runner.run()
...
def test_test_runner_error_on_timeout(mock_run: MagicMock) -> None:
    ...
    runner = CommandRunner("sleep 100", timeout=1)
    result = runner.run()
...
def test_test_runner_uses_shell(mock_run: MagicMock) -> None:
    ...
    runner = CommandRunner("echo test")
    runner.run()
...
def test_test_runner_timeout_passed(mock_run: MagicMock) -> None:
    ...
    runner = CommandRunner("test command", timeout=123)
    runner.run()
...
def test_test_runner_handles_various_exit_codes(mock_run: MagicMock) -> None:
    ...
    runner = CommandRunner("test")

    # Exit code 0 = PASS
    mock_run.return_value = MagicMock(returncode=0)
    assert runner.run() == CommandResult.PASS

    # Exit code 1 = FAIL
    mock_run.return_value = MagicMock(returncode=1)
    assert runner.run() == CommandResult.FAIL
